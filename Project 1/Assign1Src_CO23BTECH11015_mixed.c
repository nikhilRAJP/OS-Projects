...
void* runner1(void *arg){
    ...
    for(int row = param->start;row< N;row += param->jump){
        bool rowvalid=true;
        int present[N+1];
        for(int i=0;i<N+1;i++){
            present[i]=0;
...
void *runner2(void* arg){
    ...
    for(int coloumn = param->start;coloumn<N;coloumn+= param->jump){
...
void* runner3(void* arg){
    ...
    for(int grid=param->start;grid<N;grid+=param->jump){
        bool gridvalid=true;
        int present[N+1];
        for(int i=0;i<N+1;i++){
            present[i]=0;
...
int main(){
    ...
    info *info1 = (info *) malloc((P1+1)*sizeof(info));
    info *info2 = (info *) malloc((P2+1)*sizeof(info));
    info *info3 = (info *) malloc((P3+1)*sizeof(info));
